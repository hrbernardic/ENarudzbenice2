...
export interface IAuthClient {
  createUser(request: Request): Observable<FileResponse>;
  login(command: LoginCommand): Observable<LoginDto | null>;
}
...
export class AuthClient implements IAuthClient {
  ...
  constructor(
    @Inject(HttpClient) http: HttpService,
    @Optional()
    @Inject(API_BASE_URL)
    baseUrl?: string
  ) {
...
export interface IDjelatnostiClient {
  ...
  create(request: Request2): Observable<FileResponse>;
  get(id: string): Observable<FileResponse>;
  update(id: string, request: Request3): Observable<FileResponse>;
